<#
.SYNOPSIS
    Enumerates all storage accounts in a subscription or list of subscriptions and sets the blob & container soft delete to enabled with a retention time.
...
foreach ($sub in $subs) {
    try {
        ...
        try {
            ...
            $saccounts = Get-AzStorageAccount
    
            foreach ($saccount in $saccounts) {
                try {
                    $fsprop = $saccount | Get-AzStorageBlobServiceProperty
                    if ($fsprop.DeleteRetentionPolicy.Enabled -or $fsprop.ContainerDeleteRetentionPolicy.Enabled) {
                        $fsprop_issoftdeleteenabled = $fsprop.DeleteRetentionPolicy.Enabled
                    }
                    
                    #$fsprop_softdeletedays  = $fsprop.DeleteRetentionPolicy.Days   #Unused
        
                    if (!$fsprop_issoftdeleteenabled) {
                        #Soft Delete is not enabled, enable and set the retention
                        try {
                            $saccount | Enable-AzStorageDeleteRetentionPolicy -RetentionDays $RetentionDays
                            $saccount | Enable-AzStorageContainerDeleteRetentionPolicy -RetentionDays $RetentionDays
                            Write-Output ("Successfully set the soft delete for account {0}" -f $saccount.StorageAccountName)
                        }
                        catch {
